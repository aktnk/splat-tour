...
- アイコン配置によるアノテーション機能
  ...
  - アノテーションの自動永続化: 開いた3DGSファイルと同じフォルダに`<ファイル名>.annotations.json`を自動生成し、アイコンの追加・編集・削除・移動のたびに自動保存。同じファイルを再度開くと自動的に復元される（保存ボタン操作不要）

## 開発ステータス

現在、実装ロードマップの「ステップ2: AnnotationStore」「ステップ3: アイコン配置・表示（Editモード）」および、ロードマップ外で追加した「ステップ3.5: アノテーションの自動永続化」まで完了。次はステップ4「ポップアップ表示（Exploreモード）」に着手予定。
